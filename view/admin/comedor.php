...
if(isset($_POST['filtrar'])){
    ...
    if(!empty($capacidad=$_POST['capacidad_mesa'])){
        //------------    
        $comedor=$pdo->prepare("SELECT * FROM tbl_mesas WHERE capacidad_mesa=$capacidad and ubicacion_mesa='comedor'");
        //$comedor=$pdo->prepare("SELECT * FROM tbl_mesas WHERE capacidad_mesa>='{$capacidad}' order by capacidad_mesa DESC");
        $comedor->execute();
        $listaComedor=$comedor->fetchAll(PDO::FETCH_ASSOC);
        //------------
        foreach ($listaComedor as $comedor) {
            echo "<tr>";
            echo "<td><b>{$comedor['id_mesa']}</b></td>";
            echo "<td>{$comedor['capacidad_mesa']} sillas</td>"; 
            echo"<td><a href='../processes/modificar.php?email_usuario={$_SESSION['email']}' class='btnquitar'>Modificar mesa</a></td>"; 
            echo"<td><a href='../processes/eliminar.php?id_mesa={$comedor['id_mesa']}&email_usuario={$_SESSION['email']}&capacidad_mesa={$comedor['capacidad_mesa']}&ubicacion_mesa={$comedor['ubicacion_mesa']}' class='btnquitar'>Quitar reserva</a></td>";
            echo '</tr>';
        }
